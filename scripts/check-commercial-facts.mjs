...
const forbidden = [
  ...
  { re: /founding member/i, why: "founding-member lock-in language" },
  { re: /lock in/i, why: "lock-in language" },
  { re: /live nationwide/i, why: "live nationwide claim" },
...
if (contact.includes("alert(") || /Message sent! We'll respond/i.test(contact)) {
  fail("Contact still has fake alert success");
}
if (!contact.includes("submitPublicForm(\"quote_request\"")) {
  fail("Contact must submit through the governed quote_request gateway");
}
