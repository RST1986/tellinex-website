    ...
    <div className="px-4 sm:px-6 py-20 max-w-3xl mx-auto">
      <div className="text-center mb-12">
        ...
        <p style={{ fontFamily: '"Nunito", sans-serif', color: "rgba(255,255,255,0.5)", maxWidth: "580px", margin: "0 auto" }}>
          Register a service address so Tellinex can assess demand and notify you when planning reaches your area. This is not a contract or installation booking.
        </p>
      </div>
      ...
      <div style={{ background: "rgba(0,199,177,0.03)", border: "1px solid rgba(0,199,177,0.18)", borderRadius: "10px", padding: "32px", backdropFilter: "blur(8px)" }}>
        ...
        <form onSubmit={submit}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div><label style={labelStyle}>Full name *</label><input value={form.fullName} onChange={(e) => update("fullName", e.target.value)} required maxLength={120} style={inputStyle} /></div>
            <div><label style={labelStyle}>Parish *</label><select value={form.parish} onChange={(e) => update("parish", e.target.value)} required style={{ ...inputStyle, appearance: "none" }}><option value="" disabled>Select parish</option>{PARISHES.map((parish) => <option key={parish} value={parish}>{parish}</option>)}</select></div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div><label style={labelStyle}>Email</label><input value={form.email} onChange={(e) => update("email", e.target.value)} type="email" maxLength={254} style={inputStyle} /></div>
          ...
          <div className="mb-4"><label style={labelStyle}><MapPin size={12} style={{ display: "inline", marginRight: "4px" }} />Service address *</label><textarea value={form.address} onChange={(e) => update("address", e.target.value)} required minLength={5} maxLength={500} rows={3} style={{ ...inputStyle, resize: "vertical" }} /></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
            <div><label style={labelStyle}>Property type *</label><select value={form.propertyType} onChange={(e) => update("propertyType", e.target.value)} required style={{ ...inputStyle, appearance: "none" }}><option value="" disabled>Select type</option><option value="house">House</option><option value="apartment">Apartment</option><option value="business">Business</option><option value="hotel">Hotel</option><option value="other">Other</option></select></div>
            <div><label style={labelStyle}>Interest *</label><select value={form.interest} onChange={(e) => update("interest", e.target.value)} required style={{ ...inputStyle, appearance: "none" }}><option value="" disabled>Select service</option><option value="residential">Residential</option><option value="business">Business</option><option value="both">Both</option></select></div>
          </div>
