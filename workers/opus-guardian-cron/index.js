// Opus Guardian autonomous monitor — Cloudflare Worker (Cron Trigger, every 5 min).
// Ported from netlify/functions/opus-guardian-cron.mts.
// Cron schedule is defined in wrangler.toml ([triggers] crons), not in code.
//
// Env / secrets (npx wrangler secret put <NAME>):
//   SUPABASE_ANON_KEY, ANTHROPIC_API_KEY, WHATSAPP_TOKEN
//   and (legacy, see TODO) OPUS_NETLIFY_TOKEN, NETLIFY_SITE_ID_*
//
// TODO (Cloudflare migration): the health-check URLs and auto-heal actions still
// point at Netlify (tellinex-preview.netlify.app, Netlify Build API). Update the
// URLs to the Cloudflare Pages domain and replace triggerRedeploy() with a
// Cloudflare Pages deploy-hook POST once those are available.
const SUPABASE_URL = 'https://egztpclpcnizcdtfugsv.supabase.co';

async function log(env, severity, message, metadata) {
...
async function checkSupabase(env, table) {
  const SUPA_KEY = env.SUPABASE_ANON_KEY;
  try {
...
function buildChecks(env) {
  return [
    ...
    { id: 'supabase', name: 'Supabase', check: () => checkSupabase(env, 'quote_requests') },
    { id: 'fieldpack', name: 'FieldPack Sync', check: () => checkSupabase(env, 'rst_sync') },
    { id: 'opus_events', name: 'Opus Events', check: () => checkSupabase(env, 'opus_events') },
  ];
}

// ─── AUTO-HEAL ─────────────────────────────────────────────
async function triggerRedeploy(env, siteId) {
...
function buildHealMap(env) {
  return {
    website: () => triggerRedeploy(env, env.NETLIFY_SITE_ID_WEBSITE),
    landing: () => triggerRedeploy(env, env.NETLIFY_SITE_ID_LANDING),
    tcc: () => triggerRedeploy(env, env.NETLIFY_SITE_ID_TCC),
    chatbot: () => triggerRedeploy(env, env.NETLIFY_SITE_ID_WEBSITE),
  };
}
...
// ─── OPUS 4 DIAGNOSIS ──────────────────────────────────────
async function diagnoseWithOpus4(env, failing, healthy) {
...
// ─── MAIN CRON HANDLER (every 5 min, schedule in wrangler.toml) ───
async function runGuardian(env) {
  const CHECKS = buildChecks(env);
  const HEAL_MAP = buildHealMap(env);
  const results = {};
  const failing = [];
  const healthy = [];
  ...
  if (failing.length === 0) {
    await log(env, 'info', `Cron: ${CHECKS.length}/${CHECKS.length} healthy`, { results });
    return;
  }
  ...
  for (const f of failing) {
    ...
    if (healer) {
      ...
      await log(env, result.success ? 'info' : 'warning', `Auto-heal ${f.name}: ${result.success ? result.action : result.reason}`, result);
    } else {
  ...
  let diagnosis = null;
  if (unfixed.length > 0) {
    diagnosis = await diagnoseWithOpus4(env, unfixed, healthy);
    if (diagnosis) {
      await log(env, 'warning', 'Opus 4 cron diagnosis: ' + diagnosis.substring(0, 500), { unfixed: unfixed.map(f => f.id) });
    }
  }
  ...
  if (criticalFailing.length > 0) {
    ...
    await log(env, 'critical', 'WhatsApp alert sent — critical services failing', { critical: criticalFailing.map(f => f.id) });
  }

  // Step 6: Final summary log
  await log(env, unfixed.length > 0 ? 'warning' : 'info',
